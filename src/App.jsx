  ...
  return (
    <div className="min-h-screen bg-futuristic-gradient relative overflow-hidden">
      ...
      <motion.nav 
        initial={{ x: -300 }}
        animate={{ x: 0 }}
        className="hidden lg:flex fixed left-0 top-0 h-full w-80 glass-dark border-r border-[#2C3E50] z-30 flex-col"
      >
        {/* Logo/Header */}
        <div className="p-6 border-b border-[#2C3E50]">
          ...
          >
            <h1 className="text-3xl font-bold text-glow mb-2">Accountable</h1>
            <p className="text-white/60 text-sm">Financial clarity redefined</p>
          </motion.div>
        </div>
      ...
      <motion.header 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="lg:hidden fixed top-0 left-0 right-0 glass-dark border-b border-[#2C3E50] z-40"
      >
        <div className="flex items-center justify-between p-4">
          ...
          >
            {isMobileMenuOpen ? (
              <X className="w-5 h-5 text-[#BDC3C7]" />
            ) : (
              <Menu className="w-5 h-5 text-[#BDC3C7]" />
            )}
          </motion.button>
          ...
          <motion.div
            ...
            className="text-center"
          >
            <h1 className="text-xl font-bold text-glow">Accountable</h1>
          </motion.div>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="border-t border-[#2C3E50] overflow-hidden"
            >
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 glass-dark border-t border-[#2C3E50] z-40">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={() => setIsFormOpen(true)}
        className="hidden lg:flex fixed bottom-6 left-6 right-6 mx-auto w-48 bg-accent-gradient p-4 rounded-futructive shadow-futuristic-hover neon-glow items-center gap-3 justify-center z-30"
      >
        <Plus className="w-5 h-5 text-[#1A1A1A]" />
        <span className="font-semibold text-[#1A1A1A]">Add Expense</span>
      </motion.button>
