...
import AIInsights from './AIInsights'; // <- Added import
...
const Dashboard = ({ onAddExpense, onViewGoals, onViewAnalytics, onExportData, onOpenSettings }) => {
  ...
  const [activeView, setActiveView] = useState('overview'); // 'overview', 'goals', 'insights'
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter(expense => {
      if (filters.search) {
        ...
        if (!expense.description.toLowerCase().includes(searchTerm)) return false;
      }
  ...
  const cards = [
    { label: 'Total Balance', value: `$${totals.balance.toFixed(2)}`, icon: Wallet, trend: totals.balance >= 0 ? 'up' : 'down', color: totals.balance >= 0 ? 'text-success' : 'text-warning' },
    { label: 'This Month', value: `$${totals.totalIncome.toFixed(2)}`, icon: TrendingUp, trend: 'up', color: 'text-accent' },
    { label: 'Expenses', value: `$${totals.totalExpenses.toFixed(2)}`, icon: TrendingDown, trend: 'down', color: 'text-warning' },
    { label: 'Budget Left', value: `$${totals.budgetRemaining.toFixed(2)}`, icon: Target, trend: totals.budgetRemaining >= 0 ? 'up' : 'down', color: totals.budgetRemaining >= 0 ? 'text-success' : 'text-warning' },
  ];
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* Search & Filter */}
      {activeView === 'overview' && <SearchFilter onFilterChange={setFilters} />}
